...
export function AppLayout() {
  ...
  return (
    <div className="h-screen flex bg-surface-muted overflow-hidden">
      ...
      <aside
        className={cn(
          "shrink-0 bg-stone-900 text-white flex flex-col transition-all duration-200 h-full border-r border-stone-800",
          collapsed ? "w-16" : "w-64",
        )}
      >
        {/* Brand */}
          ...
          >
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-stone-400">Authority Portal</p>
              </div>
            )}
          </div>
          <button
        ...
        <div className="p-3 border-t border-stone-800 space-y-1">
          ...
          >
            ...
            {!collapsed && <UserCircle className="w-4 h-4 text-stone-500 shrink-0" />}
          </NavLink>
          <button
            type="button"
            disabled={loggingOut}
            ...
            className={cn(
              "w-full flex items-center gap-2 text-sm text-stone-400 hover:text-white py-2 px-2 rounded-lg hover:bg-stone-800 disabled:opacity-50 transition-colors",
              collapsed && "justify-center",
            )}
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-stone-200/80 bg-surface-elevated px-6 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-4">
            <NavLink
              to="/notifications"
              className="relative text-stone-400 hover:text-stone-700 transition-colors"
              title="Notifications"
            >
            ...
            <p className="text-xs text-stone-400 hidden sm:block">{user?.email}</p>
          </div>
        </header>
