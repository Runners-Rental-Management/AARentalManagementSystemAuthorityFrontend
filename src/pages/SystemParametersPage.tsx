    ...
    <div className="bg-white border border-slate-200 rounded-xl p-5 hover:border-slate-300 transition-colors">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-400 font-mono">{param.key}</p>
        </div>
        {canEdit && !editing && (
          <button
            type="button"
      ...
      {editing ? (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={saving}
              className="flex items-center gap-1 px-3 py-1.5 border border-slate-200 text-xs font-medium rounded-lg hover:bg-slate-50 transition-colors"
            >
        ...
        <div className="flex items-center justify-between">
          <span className="font-mono text-sm bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg text-slate-900">
            {param.value}
          </span>
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${!filterCat ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"}`}
        >
          All categories
        </button>
        {categories.map((c) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${filterCat === c ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"}`}
          >
            {CATEGORY_LABELS[c]}
          </button>
        ))}
